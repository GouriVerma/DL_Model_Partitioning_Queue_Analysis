...
class Simulator:
    ...
    def __init__(self, numLayers, layersFlops, layersActivationSize, gridSize, deviceToPartitionMapping, deviceComputeCapacity, linksBandwidth, env, arrival_rate=1, default_bw=1, sim_duration=50, sampling_interval=1):
        self.numLayers = numLayers
        self.layerFlops = layersFlops
        self.layersActivationSize = layersActivationSize
        self.gridSize = gridSize
        self.deviceToPartitionMapping = deviceToPartitionMapping # {device: [list of layers]}
    ...
    def simulate(self):
        ...
        def task_generator():
            """Generate tasks with Poisson arrivals to device 0."""
            ...
            while True:
                ...
                # Put task in device 0's input buffer
                # task = {"task_id": task_id, "arrival_time": self.env.now}
                record = TaskRecord(
                    task_id      = task_id,
                    origin       = (0,0),
                ...
                logging.info(f"Task {task_id} generated at time {self.env.now}")
                task_id += 1
        ...
        def periodic_sampler():
            while True:
                ...
                for device_idx in range(len(self.deviceComputeCapacity)):
                    ...
                    queue_len = len(device.input_queue.items) + len(device.compute_resource.queue)
